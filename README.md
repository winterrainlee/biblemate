...
> **Current Version: v1.3.1** (Data Integrity & Job Structural Fix)
...
- **데이터 백업**: 노트, 하이라이트, 읽기 기록을 JSON 파일(버전/메타데이터 포함)로 안전하게 내보낼 수 있습니다.
- **데이터 복구**: 백업 파일을 가져와 데이터를 복원할 수 있습니다. (5MB 제한, 데이터 무결성 검증, 결과 모달 리포트)
...
- 한글: [Fly.io 배포 가이드](docs/guides/deploy-flyio.md)
