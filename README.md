...
> **Current Version: v1.3.0** (Backup Stability & UX Improvements)
...
- **구절 다중 옵션 팝업**: 구절 클릭 시 즉시 하이라이트, 메모 작성, 인용 복사를 할 수 있는 스마트 팝업이 나타납니다.

### 📝 묵상 기록 (Writing)
- **날짜별 묵상**: 매일의 묵상을 기록할 수 있으며, 입력 중 잠시 멈추면 자동으로 저장되어 데이터 손실을 방지합니다.
- **스마트 복사**: `[날짜] 읽은 범위 + 내용` 포맷으로 깔끔하게 복사되어 나눔이나 공유가 간편합니다.
- **팝업 메모 입력**: 성경을 읽는 도중 구절 팝업을 통해 즉시 묵상을 입력하고 에디터에 자동 삽입할 수 있습니다.
- **묵상 양식 (Templates)**: '발견한 하나님', '나에게 주시는 교훈' 등 질문 기반의 양식을 제공하여 깊이 있는 기록을 돕습니다.
- **안전한 저장**: 실수로 내용을 덮어쓰거나 날리는 일이 없도록 정교한 저장 로직 및 자동 저장 기능이 적용되어 있습니다.
...
- **스마트 초기 로드**: 앱 접속 시 말라기 1장이 아닌, 사용자가 마지막으로 읽었던 구절의 다음 장을 자동으로 찾아 로드합니다.
...
### 🔒 보안 및 습관 (Security & Habit)
- **자정 세션 만료**: 매일 자정에 세션이 만료되도록 설정하여 보안을 강화하고, 매일 새롭게 접속하는 건강한 묵상 습관을 독려합니다.
- **암호 보호**: 설정된 비밀번호를 통해서만 나의 묵상 데이터에 접근할 수 있습니다. (Fly.io 배포 시 활성)
