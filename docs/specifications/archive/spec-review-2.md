...
## Review Log
- Draft: Claude Opus 4.5
- Review: GPT 5.2 thinking extend
- Review Date: 2026-01-04 (Asia/Seoul)
- Update: Step2 - 데이터 소스/적재 전략 명확화 (2026-01-04)
- Update: Step3 - Import 안전장치 + 읽기 기록 자동저장 규칙 명확화 (2026-01-04)
...
- **영어**: Open English Bible (**OEB**) 1종 (v1.0 고정)
- 기본 로딩은 **한국어(개역한글)**, 한/영 전환은 **현재 위치(책/장/절) 유지**

### 범위/식별
- 성경 **66권(구약+신약)** 지원 (외경 제외)
- 책 식별자는 **표준 코드(OSIS 등)로 내부 키 고정**
  - 화면 표시(한글/영문)는 매핑 테이블로 처리

### 진행 기록(읽기표)
- **날짜당 1개**만 저장 (같은 날짜에 다시 저장하면 덮어씀)
- 기록에는 **역본을 저장하지 않음**
...
- Export 파일명 규칙: `biblemate-backup-YYYY-MM-DD.json`  
  예) `biblemate-backup-2026-01-04.json`
...
## 📦 데이터 소스 및 적재(Import) 전략

v1.0은 **개인 단독 사용 + 자기 호스팅** 전제라서, 본문 데이터를 가져오는 경로는 “가성비”가 중요해. 다만 ‘경로’와 ‘정본(정확한 본문)’은 분리해서 생각하는 게 좋아.

### 원칙
- **정본(의도하는 텍스트)**: 한국어는 『성경전서 개역한글판』, 영어는 OEB.
- **데이터 소스(가져오는 곳)**는 비용/편의상 공개 데이터셋·깃허브·파일 등을 쓸 수 있지만, 이는 어디까지나 **운반체**야. 앱이 지켜야 할 건 “정본과 동일한 텍스트 + 출처 표기 + 무단 변경 금지(동일성 유지)”야.

### 권장 소스
- **OEB(영어)**: 공식 배포(또는 공식적으로 CC0임이 명시된 원천)에서 가져오는 걸 1순위로 둔다.
- **개역한글(한국어)**: 개인 개발 단계에서는 공개 데이터셋이 경제적일 수 있다. 단, 앱 내 출처 표기 및 동일성 유지를 위해 **원문 동일성 검증**을 거친다.

### Import 검증(최소) 체크리스트
- (1) **원천 고정**: 사용하는 데이터셋/레포는 커밋 SHA(또는 릴리스 버전)를 문서에 기록한다.
- (2) **샘플 대조**: 대표 구절 몇 개(예: 창 1:1, 시 23:1, 사 53:5, 마 5:3, 요 3:16, 롬 8:28 등)를 원문과 대조해 **완전 일치**를 확인한다.
- (3) **무단 변형 금지**: 공백/줄바꿈 정리 같은 ‘표시용’ 가공은 가능하지만, 본문 단어·구두점·절 구분을 바꾸지 않는다.
- (4) **오탈자 방지**: import 후 (version, book, chapter, verse) 키 중복/누락을 검출한다(예: 각 권의 장 수·절 수 기본 검증).

### 백업 범위(재확인)
- Export/Import(백업)은 **사용자 데이터(notes/highlights/reading_logs/settings)**만 대상으로 한다.
- `bible_verses`(본문 DB)는 용량이 크고 재생성 가능하므로 백업에서 제외한다(필요 시 별도 덤프/재적재).
...
### 2) 하이라이트
- 절 단위로 클릭 → 스타일 선택(`yellow/red/underline`) → 저장
...
**저장 규칙(사고 방지)**
- **오늘 날짜**: 책/장 범위를 바꾸면 읽기 기록을 **자동 저장**한다(예: 2~3초 디바운스, 날짜 이동/페이지 종료 직전에는 즉시 저장).
- **과거 날짜**: 탐색 중 덮어쓰기를 막기 위해 **저장 버튼(또는 편집 모드)**에서만 기록이 변경된다.
- (선택) **기록 삭제** 버튼을 제공해, 잘못 저장된 날짜를 즉시 되돌릴 수 있게 한다.
...
- 본문: 절 리스트(하이라이트 표시)
- 사이드/하단 패널: 묵상 노트(날짜별), 읽기 기록 저장 버튼
...
> 프론트 정책: **오늘 날짜**에서는 범위 변경 시 `PUT /api/reading-logs/:date`를 자동 호출하고, **과거 날짜**는 저장 버튼/편집 모드에서만 호출한다.
...
### 권장 UX 흐름(Import 안전장치)
Import는 **완전 덮어쓰기**라서, v1.0에서는 서버가 임의로 파일을 저장해두기보다(배포 환경에 따라 유실 가능) **사용자 로컬에 백업 파일을 먼저 남기는 흐름**을 기본으로 둔다.

1) Import 버튼 클릭 → **먼저 Export 실행**(`/api/backup/export`) → `biblemate-backup-YYYY-MM-DD.json` 다운로드
2) “덮어쓰기(리셋 후 복원)” 확인 모달
3) `/api/backup/import` 실행

- (선택) 추가 안전장치가 필요하면, 서버는 최근 1개 스냅샷을 **DB 테이블**로만 보관한다(파일시스템 의존 금지).
...
- 처리 순서(권장): (클라이언트) 사전 Export → (서버) 유효성 검사 → 트랜잭션 시작 → 테이블 초기화 → 삽입 → 커밋
...
### Phase 1: MVP (v1.0)
1) DB 스키마/마이그레이션
2) 성경 본문 로딩 API + 범위 로딩 UI
3) 하이라이트 저장/로드
4) 노트 저장/로드
5) 읽기 기록 저장 + 달력 표시
6) Export/Import
7) 설정(테마/글자 크기) + 마지막 위치 저장
