...
- **본문 글꼴과 UI 글꼴 분리** `v2.3`
  - 독서 앱에서 세리프 본문은 정서와 가독성에 직접 영향을 주지만, 버튼/탭/설정까지 모두 세리프로 바꾸면 조작성이 떨어질 수 있습니다.
  - **Lesson**: `--pk-font-family`는 UI용 산세리프로 유지하고, 성경 본문은 `--pk-font-body` 같은 전용 변수로 분리하면 독서 경험과 도구 경험을 동시에 살릴 수 있습니다.
...
### 🧭 동일 UI 패턴의 화면 범위 확인
- **성경읽기 모드 vs 묵상일지 모드** `v2.1.4`
  - "묵상 카드", "개별 묵상", "묵상모드"처럼 사용자 표현이 겹칠 때, 실제 의도는 특정 화면(예: 성경읽기 모드 사이드바 카드)이 우선일 수 있다.
  - **Lesson**: 같은 UI 패턴이 여러 화면에 존재하면 구현 전 관련 컴포넌트를 모두 찾고, 주 대상 화면과 부가 적용 화면을 코드 기준으로 명시하라. 특히 `BibleViewer`와 `JournalPage`처럼 비슷한 카드 UX가 분리되어 있으면 한쪽만 수정해 배포본에서 누락처럼 보일 수 있다.
...
## 17. 모바일 묵상 흐름 재설계 `v2.2.0`

### 📱 데스크탑 패널은 모바일 전용 흐름으로 변환해야 한다
- **문제**: 데스크탑의 3컬럼 묵상 구조는 강력하지만, 모바일에서 단순히 사이드바를 숨기면 현재 장 묵상 맥락이 사라진다.
- **Lesson**: 반응형 작업에서는 `display: none`으로 사라지는 기능의 역할을 먼저 정의하고, 모바일에서는 바텀시트·전체 화면 작성·하단 action bar 같은 전용 흐름으로 변환하라.

### 📝 묵상 작성 화면에는 선택한 말씀이 항상 보여야 한다
- **문제**: 사용자가 구절을 선택해 묵상을 쓰는 순간, 말씀 본문이 사라지면 묵상 작성의 기준점도 사라진다.
- **Lesson**: 구절 묵상 작성 UI는 입력창보다 먼저 `본문 참조 + 선택한 말씀 내용`을 안정적으로 보여줘야 한다. 특히 모바일 전체 화면 작성 모드에서는 이 영역을 상단 맥락으로 고정하라.

### 🧭 읽기표는 통계가 아니라 다음 행동의 출발점이어야 한다
- **문제**: 진행률만 보여주는 읽기표는 모바일에서 다음 읽기 행동으로 이어지기 어렵다.
- **Lesson**: 작은 장 셀을 모두 버튼화하기보다 `다음 안 읽은 장 읽기`와 책 row 진입점을 제공하면 접근성 44px 기준과 행동성을 동시에 확보할 수 있다.

### 🔤 앱 전체 글자 크기와 본문 가독성은 분리해야 한다
- **문제**: 성경 본문 가독성을 위해 root font-size를 바꾸면 헤더, 버튼, 시트 레이아웃까지 함께 흔들릴 수 있다.
- **Lesson**: 성경 본문은 별도 CSS 변수나 scale을 사용해 앱 chrome과 독립적으로 조절하라.

---

## 18. 모바일 선택 모드 공간 관리 `v2.3.2`

### 📱 선택 모드에서는 주변 chrome을 접어야 한다
- **문제**: 모바일 성경 화면에서 구절 선택 팝업이 뜰 때 상단 헤더와 하단 액션 바가 동시에 남아 있으면 실제 선택/작성 공간이 좁아진다.
- **Lesson**: 모바일 선택 모드처럼 한 작업에 집중해야 하는 순간에는 헤더와 하단 바를 `selection-mode`/`selection-hidden` 같은 명시적 상태 클래스로 접어, 콘텐츠 조작 영역을 우선 확보하라.

### 🧾 상태 정보는 행을 늘리기보다 문맥 옆에 붙인다
- **문제**: `읽음/읽지 않음` 같은 짧은 상태를 별도 행으로 두면 모바일 헤더가 쉽게 3~4행으로 커진다.
- **Lesson**: 짧은 상태는 제목이나 현재 위치 옆의 badge로 통합하고, 주요 액션은 별도 compact action row로 묶으면 기능을 잃지 않으면서 세로 공간을 줄일 수 있다.

---

## 19. 묵상 기록과 읽음 기록의 데이터 일관성 `v2.3.3`

### 🧩 파생 기록은 서버 저장 흐름에서 보강한다
- **문제**: 사용자가 구절 묵상을 남기는 것은 해당 장을 실제로 읽었다는 강한 신호지만, 수동 읽음 버튼을 누르지 않으면 캘린더와 묵상일지의 읽음 기록이 비어 보일 수 있다.
- **Lesson**: 한 사용자 행동이 다른 핵심 기록을 자연스럽게 암시할 때는 프론트 UI만 보정하지 말고 서버 저장 흐름에서 파생 기록을 생성해 데이터 일관성을 확보하라. 단, 기존 기록 포함 여부를 먼저 확인해 중복 생성을 막아야 한다.
