...
- `[Minor]` `🟡 Medium` **다중 하이라이트**: 한 구절에 여러 색상/스타일 겹쳐서 표시
- `[Minor]` `🟡 Medium` **고급 검색**: AND/OR 조건, 초성 검색, 검색 범위 지정
- `[Patch]` `🟢 Easy` **읽기 완료 토글 개선**: 버튼 토글 로직 정교화 및 애니메이션 효과 추가
- `[Minor]` `🟢 Easy` **화면 분할 비율 조정**: 데스크탑/태블릿에서 말씀/묵상 영역 크기 조절

### 📝 노트 (Notes)
- `[Minor]` `🟡 Medium` **마크다운 노트 지원**: 볼드, 이탤릭, 리스트 등 텍스트 서식 지원

### 📊 진도 (Tracking)
- `[Minor]` `🟢 Easy` **읽기표 페이지 전환**: 팝업에서 전용 페이지로 변경 (시각화 개선)
- `[Minor]` `🟡 Medium` **통계 기능 강화**: 월별 독서량 그래프, 성경 전체 진도율 시각화
- `[Minor]` `🟡 Medium` **통독 플랜 (Reading Plans)**: 1년 1독, 맥체인 등 읽기표 템플릿 및 진도 체크

### ⚙️ 설정 (Settings & General)
- `[Minor]` `🔴 Hard` **오프라인 지원 (PWA)**: 인터넷 없이 동작 (Service Worker)
