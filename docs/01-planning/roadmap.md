...
### 🐞 v1.4.1 버그 수정 (Patch 우선 처리)
- `[Patch]` `🟢 Easy` **모바일 본문-네비게이션 겹침 버그**: 말씀 본문이 성경 네비게이션과 일부 겹쳐 보이는 문제 (CSS 수정)
- `[Patch]` `🟢 Easy` **모바일 본문-묵상 공백 버그**: 말씀 본문과 묵상기록 구역 사이 불필요한 공백 (CSS 수정)
- `[Patch]` `🟢 Easy` **HTML 엔티티 표시 버그**: 성경 본문에서 `&#x27;` 등이 그대로 나타남 (누가복음 5장 등)
...
- `[Minor]` `🟢 Easy` **모바일 달력 숨김 옵션**: 오늘 말씀에 집중할 수 있도록 토글 제공
- `[Minor]` `🟢 Easy` **설정 페이지 헤더 구조**: 성경 읽기표처럼 헤더가 있는 구조로 변경
- `[Minor]` `🟢 Easy` **설정 페이지 항목 순서 조정**: 테마 제거(상단 아이콘 중복) 및 우선순위 기반 정렬
