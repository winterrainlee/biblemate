...
- [ ] `🟡 Medium` **라이트/다크 Reading Canvas 검증**
...
- [ ] `🔴 Hard` **기기명 대신 사용 가능한 폭 기준으로 반응형 구조 재설계**
- [ ] `🟡 Medium` **Compact `< 약 600px`**: 단일 본문 + 엄지 영역 중심 조작
- [ ] `🟡 Medium` **Reading `약 600–899px`**: 외부 기록 앱과 1:1 병행 가능한 단일 본문
- [ ] `🟡 Medium` **Workspace `약 900px 이상`**: 본문 + 필요 시 우측 묵상 패널
- [ ] `🟡 Medium` **769~1024px 진입점 사각지대 제거**
- [ ] `🟡 Medium` **기존 `묵상 | 본문 | 묵상` 3열 구조 제거**
...
- [ ] `🔴 Hard` **탭 기반 단일/다중 구절 선택 모델**
- [ ] `🟡 Medium` 선택 추가/해제 및 비연속 선택
- [ ] `🟡 Medium` Selection 상태에서 장 이동 스와이프 억제
- [ ] `🟡 Medium` 선택 시 본문 위치 유지
- [ ] `🟡 Medium` 주요 터치 hit area 44×44px 이상 확보

### 4. Context Toolbar

- [ ] `🔴 Hard` **본문을 덮지 않는 Context Toolbar 도입**
- [ ] `🟡 Medium` 하이라이트 4색 연결
- [ ] `🟡 Medium` 묵상 Composer 진입
- [ ] `🟡 Medium` 복사
- [ ] `🟢 Easy` 더보기 진입점
...
- [ ] `🟡 Medium` 단일 절 출처 표기
- [ ] `🟡 Medium` 연속 다중 절 범위 표기 (`1:3–5`)
- [ ] `🟡 Medium` 비연속 다중 절 표기 (`1:3, 5`)
- [ ] `🟢 Easy` 현재 역본명 포함
- [ ] `🟡 Medium` 선택 범위와 실제 복사 본문 정합성 검증
...
- [ ] `🔴 Hard` **Compact 전체 화면 Composer**
- [ ] `🔴 Hard` **Workspace 우측 패널 Composer**
- [ ] `🟡 Medium` 선택 구절/인용문 명확히 표시
- [ ] `🟡 Medium` 작성 중 이탈 보호
- [ ] `🟡 Medium` iOS 키보드 + safe-area 검증
- [ ] `🟡 Medium` 기존 `verse_notes` API/데이터 재사용
...
- [ ] `🟡 Medium` 모든 폭에서 묵상 접근 경로 유지
- [ ] `🟡 Medium` Workspace 우측 묵상 패널
- [ ] `🟡 Medium` 패널 접기/펼치기
- [ ] `🟡 Medium` 묵상 보기/수정/삭제 액션 재배치
- [ ] `🟡 Medium` 본문 구절로 이동 동작 유지
...
- [ ] `🟡 Medium` 중첩 card/surface 제거
- [ ] `🟡 Medium` shadow 사용 위치 제한
- [ ] `🟡 Medium` radius 사용 규칙 통일
- [ ] `🟡 Medium` border보다 여백/타이포그래피 우선
- [ ] `🟡 Medium` Header 시각적 무게 축소
- [ ] `🟡 Medium` Reading과 Journal의 디자인 언어 정합
...
1. [ ] Journal
2. [ ] Reading History / Chart
3. [ ] Settings
4. [ ] Login
5. [ ] PWA / favicon / app icon 정합성 확인
...
- [ ] Reading → Selection → 흔적 남기기 → Reading 루프가 v2.3보다 짧고 안정적이다.
- [ ] iPhone 13 mini급 폭 검증 완료.
- [ ] iPad/desktop 1:1 Split View급 폭 검증 완료.
- [ ] Desktop Workspace 검증 완료.
- [ ] 기존 묵상/하이라이트/읽기 기록 보존.
- [ ] Backup/Restore 호환성 유지.
- [ ] 각 feature별 Implementation Plan / Walkthrough / PR 기록 완료.
- [ ] 최종 lint/build 및 수동 회귀 검증 완료.
- [ ] `release-notes-v3.0.0.md` 완료.
