...
- [ ] 현재 하단 선택 bar를 상태 행 + 액션 행의 Context Toolbar로 확장
- [ ] 액션 행에 하이라이트 4색, 묵상, 복사를 직접 노출해 구절 선택 후 한 번의 추가 터치로 실행
- [ ] `더보기`에는 하이라이트 지우기를 두고, 닫기는 상태 행에 독립적으로 유지
- [ ] 색상 버튼은 선택 전체를 해당 색으로 **설정**하고, 같은 색을 다시 눌러도 지우지 않음
- [ ] 하이라이트 지우기는 선택 중 실제 하이라이트가 있는 절에만 적용
- [ ] 하이라이트·복사 성공 시 선택을 닫고 Reading으로 복귀, 실패 시 선택을 유지해 재시도
- [ ] 복사 본문은 단일 절은 본문만, 다중 절은 각 줄 앞에 절 번호를 붙이는 형식
- [ ] 묵상 버튼은 선택 범위와 인용문을 기존 작성 popup에 전달하되 Composer 재설계는 다음 단계로 유지
- [ ] 이번 작업의 검수 범위: iPhone 모바일 승인을 통합 게이트로 사용할지, 명세 기본값대로 Desktop 조합까지 요구할지 최종 확인
...
2. 첫 행에는 선택 개수·범위, `더보기`, 닫기를 배치한다.
3. 둘째 행에는 하이라이트 4색, 묵상, 복사를 배치한다.
...
3. 색상 재선택을 삭제 동작으로 사용하지 않는다. 삭제는 `더보기 → 하이라이트 지우기`로만 수행한다.
...
- 묵상, 복사, 더보기, 닫기 버튼과 pending/error 상태를 연결한다.
...
Selection
  ...
  ├─ 더보기 → MoreMenu → 지우기 → RemovingHighlight → 성공/실패 처리
...
이 항목을 모바일 승인만으로 대체하려면 구현 승인 시 기존 명세의 검증 예외를 이번 작업까지 확장한다고 명시적으로 확인한다.
...
- **작은 화면 toolbar 과밀**: 상태/보조 액션 행과 핵심 액션 행을 분리하고 44px hit area를 먼저 확보한다.
