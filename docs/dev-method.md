...
4.  **명세서 작성 (Specification)**: 확정된 범위를 바탕으로 `spec-v버전명.md`를 작성하여 구현 상세와 기술적 변경 사항을 정의합니다.
...
모든 작업(Feature/Bug fix) 브랜치는 다음의 **5단계 승인 게이트**를 거칩니다.

### 1단계: 구현 계획 작성 (Implementation Planning)
- 코드 수정 전, AI가 다음 내용을 포함한 `implementation_plan.md`를 작성하여 제출합니다.
...
- **변경 로그 기록**: 합의된 모든 변경 사항은 `dev-log`에 한 줄로 남깁니다.
