...
- 헤더·탭·사이드바에서 시작되는 화면 전환도 공용 navigation guard로 보호합니다.
- 저장 성공과 후속 목록 refresh를 분리해 성공한 저장을 실패로 오인하지 않습니다.

## 주요 변경

- Compact 전체 화면, Reading dialog, Workspace 우측 1/3 Composer
- 기존 날짜와 `verse_range`를 보존하는 edit payload
- 선택 범위·인용문·dirty 비교와 navigation guard 순수 모델 및 Node 테스트 9건
...
- [x] `npm run lint`
- [x] `npm run build`
- [x] `node --test src/components/reflectionComposerModel.test.js src/contexts/navigationGuard.test.js` — 9/9
